...
export default abstract class Entity extends Body {
    ...
    isOnGround: () => boolean = () => {
        const colliders = this.game.world.getBodiesCollidingWith(this.expand(4))
    ...
    nextPhysicsTick(deltaTime: number) {
        if (!this.isOnGround() || Math.abs(this.xVelocity) > 0 || Math.abs(this.yVelocity) > 0) {
            ...
            const xCollisions = this.game.world.getRangeBodies(potentialX, this.y, potentialX + this.width, this.y + this.height)
            const yCollisions = this.game.world.getRangeBodies(this.x, potentialY, this.x + this.width, potentialY + this.height)
